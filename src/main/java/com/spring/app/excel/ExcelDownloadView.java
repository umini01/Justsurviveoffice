...
public class ExcelDownloadView extends AbstractView {
	...
	protected void renderMergedOutputModel(Map<String, Object> model, 
     ...
     // 여기서부터는 각 브라우저에 따른 파일이름 인코딩작업이다.
        ...
        workbook = (SXSSFWorkbook) model.get("workbook"); // 넘겨받은 모델(다운로드되어질 엑셀파일의 정보)
        ...
        workbook.write(ost); // 엑셀파일 생성
        
        workbook.close(); // 엑셀파일생성에 사용했던 SXSSFWorkbook workbook 은 역할을 다했으므로 메모리 관리를 위해 close로 메모리상에서 삭제하도록 한다.
        ost.close(); // 엑셀파일생성에 사용했던 OutputStream ost 는 역할을 다했으므로 메모리 관리를 위해 close로 메모리상에서 삭제하도록 한다.
	}
